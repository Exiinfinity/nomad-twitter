...
export default function Enter() {
  ...
  return (
    <div className="">
      <h3 className="text-3xl font-bold text-center pt-14 pb-5">
        로그인
      </h3>
        ...
        {data?.ok ? (
          ...
          >
            <Input
              ...
              required
            />
            <Button text={loading ? "로딩중.." : "토큰 확인하기"} />
          </form>
        ) : (
          <form className="flex flex-col px-4" onSubmit={handleSubmit(onValid)}>
            <Input
              ...
              required
            />
            <Button text={"로그인 링크 가져오기"} />
          </form>
        )}
      </div>
      <div className="px-4">
        <Button text={"회원가입"} secondColor onClick={onClick} />
      </div>
    </div>
  );
}
